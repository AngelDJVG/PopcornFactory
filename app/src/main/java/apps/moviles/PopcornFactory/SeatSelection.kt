package apps.moviles.PopcornFactory

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.widget.Toast
import apps.moviles.PopcornFactory.databinding.ActivitySeatSelectionBinding
...
class SeatSelection : AppCompatActivity() {
    ...
    fun deshabilitarAsientos(posMovie: Int){
        for (boleto in CatalogoPeliculas.catalogo.get(posMovie).seats) {
            ...
            val seatButton = findViewById<RadioButton>(boleto.asiento)
            seatButton.isEnabled = false
            val drawable = ContextCompat.getDrawable(this, R.drawable.radio_selectable)
            seatButton.background = drawable
        }
    }
}